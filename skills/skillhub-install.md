...
- **❌未启用** → **不要说"我没有终端能力"**；如实告诉用户：「安装技能需要终端执行能力，当前未启用。请在配置 `agent.sandbox.mode: e2b` 并填 `apiKey` 后发 `#agents重载`，再让我安装。」若盘点里给出了失败原因，一并转述。然后停止。
...
2. **安装 CLI（仅未装时）**——需联网（命令在沙箱内直接执行，无审批）：
...
3. **安装指定技能**——`--dir` 必须指向【运行能力盘点】里的技能目录：
   ```
   skillhub install <技能名> --dir <技能目录>
   ```
   搜索（只读）：`skillhub search <关键词>`。

4. **安装后热加载**（免重启）：调用 `reload_skills` 工具重新扫描技能目录，告知用户已加载。

## 重要约束

- **命令在 E2B 沙箱内直接执行，无审批**：写类命令（`curl|bash`、`skillhub install`）与只读命令一样，提交后立即在微虚机里跑，不会出现 `#确认/#拒绝`。
...
2. 未装则 `terminal({command: "curl -fsSL https://skillhub-1388575217.cos.ap-guangzhou.myqcloud.com/install/install.sh | bash"})`。
3. `terminal({command: "skillhub install find-skill-skillhub --dir <技能目录>"})`。
