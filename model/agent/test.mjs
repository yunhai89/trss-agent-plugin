...
await test('policy：decide 决策矩阵 + 群管放行', async () => {
  ...
  // 群管工具免审批（meta.groupAdminSkip）：群管理/群主调用群管动作（kick/mute 等 system 类高危）
  // 直接执行——权限来自群本身（QQ 侧本就有这些权限），无需主人 #确认
  const { groupKickTool, groupMuteTool, groupSetCardTool } = await import('../group/manage.js')
  eq(decide({ role: 'admin', isMaster: false, isGroup: true, isGroupAdmin: true }, groupKickTool).decision, 'allow', '群管理员→group_kick 直接执行（不走主人审批）')
  eq(decide({ role: 'owner', isMaster: false, isGroup: true, isGroupAdmin: true }, groupMuteTool).decision, 'allow', '群主→group_mute 直接执行')
  eq(decide({ role: 'member', isMaster: false, isGroup: true, isGroupAdmin: false }, groupKickTool).decision, 'deny', '普通成员仍 deny')
  eq(decide({ role: 'admin', isMaster: false, isGroup: false, isGroupAdmin: false }, groupKickTool).decision, 'deny', '私聊场景不放行（isGroup=false）')
  eq(decide({ role: 'admin', isMaster: false, isGroup: true, isGroupAdmin: true }, { category: 'system' }).decision, 'deny', '其他 system 工具不受影响（无 groupAdminSkip 标记）')
  eq(decide({ role: 'member', isMaster: false, isGroup: true, isGroupAdmin: true }, groupSetCardTool).decision, 'allow', 'group_manage 类工具群管放行回归')
  const gpOn = createPolicy({})
  eq(gpOn.decide({ role: 'admin', isMaster: false, isGroup: true, isGroupAdmin: true }, groupKickTool).decision, 'allow', 'createPolicy 同款放行')
  const gpOff = createPolicy({ groupAdminSkip: false })
  eq(gpOff.decide({ role: 'admin', isMaster: false, isGroup: true, isGroupAdmin: true }, groupKickTool).decision, 'deny', 'groupAdminSkip:false 可关闭（回 deny）')
