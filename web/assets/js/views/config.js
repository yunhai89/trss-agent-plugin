  ...
  window.VIEWS.config = {
    ...
    setup() {
      ...
      const syncForm = (snap) => {
        ...
        if (!Array.isArray(form.humanize.knownBots)) form.humanize.knownBots = []
        if (!form.groupWorld) form.groupWorld = {}
        if (!form.groupWorld.graph) form.groupWorld.graph = {}
        if (!form.groupWorld.retrieval) form.groupWorld.retrieval = {}
        if (!form.selfState) form.selfState = {}
        for (const k of ['scope', 'eventDetection', 'emotion', 'resentment', 'expectations', 'reflection', 'planner', 'replyer', 'stability', 'retention']) {
          if (!form.selfState[k] || typeof form.selfState[k] !== 'object') form.selfState[k] = {}
        }
      ...
      // 对象型高级配置（policy.categoryMin / media.caps）以 JSON 文本编辑；解析失败保持原值
      const updateJson = (path, str) => {
    ...
    <div class="cf-wrap">
      ...
      <div>
        ...
        <div :id="'cfg-sandbox'" class="card cf-sec" :class="{open: open.sandbox}">
          <div class="cf-sh" @click="open.sandbox = !open.sandbox">
            <span class="ct-ico" style="background:var(--grad-mint)"><v-icon name="tool"/></span>
            <div><div class="ct-t">E2B 沙箱（终端执行）</div><div class="ct-s">命令在 Firecracker microVM 内执行 · 无审批 · 出口白名单 · fail-closed</div></div>
            <v-icon class="cf-arrow" name="chevron"/>
          </div>
          <div class="cf-body" v-show="open.sandbox"><div class="cf-grid">
            <div class="desc mb10">命令跑在独立 Firecracker microVM 内，宿主不再有 shell 执行面。沙箱化后<b>无审批、无命令黑名单</b>，所有用户可用（按会话隔离沙箱 + 单会话命令数/并发/超时上限兜底成本）；连不上 E2B 一律拒绝执行，绝不回退本机。</div>
